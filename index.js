...
//Get task request
app.get("/task", authMiddleware, sync(req, res) => {
  const Task = await Task.find({userId: req.user_id})
  res.json(Task);
...
app.patch("/tasks/:id?priority",authMiddleware,async(req,res)=>{
  ...
  const task=await Task.findOneAndUpdat(
{_id:req.params.id,userId: userId:userId},
{priority},
{new:true }
  );
  if(!task)return res.status(404).json({message:"Task not found" });
